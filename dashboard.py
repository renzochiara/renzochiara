...
def check_password():
    return st.secrets["EMAIL"] == st.session_state.email and st.secrets["PASSWORD"] == st.session_state.password
...
if not st.session_state.logged_in:
    ...
    page = st.sidebar.radio("Go to", ["Chat with AI", "Upload & Process Files", "Run Workflow"])
    ...
    # --- Run Workflow ---
        ...
        if st.button(f"Generate Budget for {selected_parking}"):
            if uploaded_excel and uploaded_pdf:
                ...
                # Fill K17-K26 (example)
                try:
                    ws["K17"] = float(df_pnl.loc[df_pnl.iloc[:, 0] == "Transient Revenue", df_pnl.columns[1]].values[0])
                except:
                    ws["K17"] = 0
                try:
                    ws["K18"] = float(df_pnl.loc[df_pnl.iloc[:, 0] == "Monthly Revenue", df_pnl.columns[1]].values[0])
                except:
                    ws["K18"] = 0
                try:
                    ws["K26"] = float(df_pnl.loc[df_pnl.iloc[:, 0] == "TOTAL REVENUE", df_pnl.columns[1]].values[0])
                except:
                    ws["K26"] = 0
                ...
                # Apply supervisor costs (example)
                hourly_rate = 25.0  # Default rate
                monthly_cost = hourly_rate * supervisor_hours * 30  # Approximation
                if "4-Feuille calcul-Salaire" in wb.sheetnames:
                    ws_salary = wb["4-Feuille calcul-Salaire"]
                    ws_salary["D10"] = monthly_cost  # Example cell for supervisor cost
